...
class amc_form(QtWidgets.QMainWindow,Ui_MainWindow):
    def __init__(self):
        ...
        #self.light_lable.setStyleSheet("border: 2px solid red")
        self.light_lable.setPixmap(pix)


        self.model=QStandardItemModel(4,4)
        self.model.setHorizontalHeaderLabels(['表号','编号','名称','类型'])#,'符号'

        self.mode2=QStandardItemModel(5,5)
        self.mode2.setHorizontalHeaderLabels(['表号','编码号','类型','参数名称','结果'])


        self.InfoKeyData={}#存储配置文件使用字典类型
        #载入配置文件
        self.LoadXML()
        ...
        self.Listening_thread=self.Listening_Thread(self.light_lable)
    ...
    def LoadXML(self):
        ...
        for protocol in Protocols:
            if protocol.hasAttribute("LocalSysCode"):
                #print("LocalSysCode:", protocol.getAttribute("LocalSysCode"))
                #print("LocalNodeCode:", protocol.getAttribute("LocalNodeCode"))
                #print("RemoteSysCode:", protocol.getAttribute("RemoteSysCode"))
                #print("RemoteNodeCode:", protocol.getAttribute("RemoteNodeCode"))
        ...
        for cmd1 in Cmd1s:
            if cmd1.hasAttribute("SendPort"):
                #print("SendPort:", cmd1.getAttribute("SendPort"))
                #print("TargetPort:", cmd1.getAttribute("TargetPort"))
                #print("RevDataPort:", cmd1.getAttribute("RevDataPort"))
                #print("HeartBeatPort:", cmd1.getAttribute("HeartBeatPort"))
                #print("TartGetIP1:", cmd1.getAttribute("TartGetIP1"))
                #print("TartGetIP2:", cmd1.getAttribute("TartGetIP2"))
                ...
                print(type(cmd1.getAttribute("TartGetIP2")))
                print(type(int(cmd1.getAttribute("SendPort"))))
                print(int(self.InfoKeyData['SendPort'][0]))
        ...
        for cmdtemp in Cmds:
            ...
            #print("CmdS:", cmdtemp.getAttribute("Code"))
            #print("CmdS:", cmdtemp.getAttribute("Name"))
            Cmd_list = []
            for i in range(4):
        ...
        for i,x in enumerate(Datas):
            ...
            for j_col in range(4):
                item=QStandardItem(Data_list[j_col])
                self.mode2.setItem(i, j_col, item)
            #self.tableView_data.setRowHeight(i,60)

    def machine_1_clicked(self):
        #print("machine_1 clicked")
        global TargetAddr_global
        TargetAddr_global=self.TargetAddr_1

    def machine_2_clicked(self):
        #print("machine_2 clicked")
        global TargetAddr_global
    ...
    def tableview_clicked(self,index):
        ...
        if rsp == QtWidgets.QDialog.Accepted:
            ...
            #发送网络指令
            #QMessageBox.information(self,'ok',self.para_edit.text())
            udp_socket_=socket(AF_INET,SOCK_DGRAM)
            #print("step8")
            #_b=self.intToBytes(3003,4) #bytes数组发送


            #print("Y/M/D",self.InfoHead_Year,self.InfoHead_Month,self.InfoHead_Day)
            #print("MCS_Global_lastFrameCount=", MCS_Global_lastFrameCount)


            #QMessageBox.information(self, 'ok', data.to_bytes(4, byteorder='little', signled=True))
            #for i in range(10):
            #   #str(_a).encode("utf-8")
...
#or (addr[0]==self.TargetAddr_global[0])==self._radiobutton_2.isChecked()
